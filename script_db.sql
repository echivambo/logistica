...
create view control_p as
 select
  ...
  sum(e.quantidade-c.quantidade) as consumo,
...
create view control_p as
 select
  ...
  e.mes as mes,
   sum(c.quantidade) as contagem_fisica,
  sum(e.quantidade) as qtd_entrada,
  s.saldo_inicial as saldo_inicial,
...
SELECT mes, franquia, produto, codigo, consumo FROM `control_p` 
UNION
SELECT mes, franquia, produto, saldo_inicial, codigo FROM saldo_inicial
